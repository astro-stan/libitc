...
void ITC_Event_Test_normaliseEventIsRecoveredOnFailure(void)
{
    ...
    /* Test failing to normalise a (MAX_EVENT_COUNT, 1, 2) Event */
    gpt_ParentEvent->t_Count = ~((ITC_Event_Counter_t)0);
    gpt_ParentEvent->pt_Left->t_Count = 1;
    gpt_ParentEvent->pt_Right->t_Count = 2;
    TEST_FAILURE(
        ITC_Event_normalise(gpt_ParentEvent),
        ITC_STATUS_EVENT_COUNTER_OVERFLOW);

    /* Test the Event is the same */
...
void ITC_Event_Test_maximiseEventIsRecoveredOnFailure(void)
{
    ...
    /* Test failing to maximise a (MAX_EVENT_COUNT, 3, 0) Event */
    gpt_ParentEvent->t_Count = ~((ITC_Event_Counter_t)0);
    gpt_ParentEvent->pt_Left->t_Count = 3;
    gpt_ParentEvent->pt_Right->t_Count = 0;
    TEST_FAILURE(
        ITC_Event_maximise(gpt_ParentEvent),
        ITC_STATUS_EVENT_COUNTER_OVERFLOW);
